...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Nirob-vai/NI4OB-CYBER-PAID/tree/main').text
			if basesplit in plr:
				key = basesplit
...
#				exit()
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [NIROB] : \033[0;97m %s/%s -> \033[0;92m [NIROB -OK:%s ]- \033[0;93m[NIROB-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
